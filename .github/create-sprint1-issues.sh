#!/bin/bash
# Script to create all Sprint 1 issues in GitHub

REPO="M-Boiguille/SkillOps"

# Create milestone
echo "Creating Sprint 1 milestone..."
MILESTONE_NUMBER=$(gh api repos/$REPO/milestones -f title="Sprint 1 - MVP Core" \
  -f state="open" \
  -f description="Développer un CLI fonctionnel avec WakaTime, Review, Reinforce, Persistence et CI/CD. Objectif: 19 story points en 2 semaines (9-23 jan 2026)" \
  -f due_on="2026-01-23T23:59:59Z" \
  --jq '.number')
...
gh issue create \
  --title "T010-1: Créer structure storage/ avec .gitignore" \
  --body "**User Story:** US-010 - Persistence des Données
...
gh issue create \
  --title "T010-2: Implémenter StateManager pour gestion YAML" \
  --body "**User Story:** US-010 - Persistence des Données
...
gh issue create \
  --title "T010-3: Implémenter ProgressManager pour gestion JSON" \
  --body "**User Story:** US-010 - Persistence des Données
...
gh issue create \
  --title "T010-4: Implémenter MetricsManager pour calculs agrégés" \
  --body "**User Story:** US-010 - Persistence des Données
...
gh issue create \
  --title "T011-1: Setup projet Python avec dépendances" \
  --body "**User Story:** US-011 - Interface CLI Interactive
...
gh issue create \
  --title "T011-2: Implémenter menu principal interactif" \
  --body "**User Story:** US-011 - Interface CLI Interactive
...
gh issue create \
  --title "T011-3: Implémenter affichage Rich (tableaux, couleurs)" \
  --body "**User Story:** US-011 - Interface CLI Interactive
...
gh issue create \
  --title "T011-4: Tests CLI (inputs/outputs)" \
  --body "**User Story:** US-011 - Interface CLI Interactive
...
gh issue create \
  --title "T002-1: Implémenter step Review (récupération données)" \
  --body "**User Story:** US-002 - Review des Métriques Quotidiennes
...
gh issue create \
  --title "T002-2: Affichage formaté Rich pour métriques" \
  --body "**User Story:** US-002 - Review des Métriques Quotidiennes
...
gh issue create \
  --title "T002-3: Tests step Review" \
  --body "**User Story:** US-002 - Review des Métriques Quotidiennes
...
gh issue create \
  --title "T001-1: Setup WakaTime API client" \
  --body "**User Story:** US-001 - Tracking de Formation (WakaTime)
...
gh issue create \
  --title "T001-2: Intégration WakaTime dans step Formation" \
  --body "**User Story:** US-001 - Tracking de Formation (WakaTime)
...
gh issue create \
  --title "T001-3: Configuration secrets (.env)" \
  --body "**User Story:** US-001 - Tracking de Formation (WakaTime)
...
gh issue create \
  --title "T001-4: Tests WakaTime avec mocks" \
  --body "**User Story:** US-001 - Tracking de Formation (WakaTime)
...
gh issue create \
  --title "T003-1: Implémenter step Reinforce (exercices)" \
  --body "**User Story:** US-003 - Exercices de Renforcement
...
gh issue create \
  --title "T003-2: Sauvegarde progression exercices" \
  --body "**User Story:** US-003 - Exercices de Renforcement
...
gh issue create \
  --title "T003-3: Tests step Reinforce" \
  --body "**User Story:** US-003 - Exercices de Renforcement
...
gh issue create \
  --title "T-CI-1: Créer GitHub Actions CI pipeline" \
...
gh issue create \
  --title "T-CI-2: Configurer pre-commit hooks" \
  --body "**Catégorie:** Infrastructure & CI/CD
...
gh issue create \
  --title "T-CI-3: Ajouter badges CI/CD au README" \
  --body "**Catégorie:** Infrastructure & CI/CD
...
echo "✓ Milestone: Sprint 1 - MVP Core (due 2026-01-23)"
echo ""
echo "Next steps:"
echo "1. Create GitHub Project Board"
echo "2. Add issues to project"
